...
case class Voucher(id: Long, discount: Double, voucherType: VoucherType, active: Boolean)
...
class VoucherTable(tag: Tag) extends Table[Voucher](tag, "Vouchers") {
  ...
  def * = (id, discount, promotionType, active) <> ((Voucher.apply _).tupled, Voucher.unapply)
}